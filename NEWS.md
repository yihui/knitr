...
- `include_graphics()` now converts absolute paths to paths relative to the output directory of the rendered document (communicated by **rmarkdown** >= 2.32 via `opts_knit$get('rmarkdown.output_dir')`) instead of knitr's working directory (the input directory), which fixes broken image paths when the input and output directories differ (e.g., rendering to a different `output_dir`). The existence check (when `error = TRUE`) considers a file found if it exists under either the original path or the converted (relative) path, so it no longer reports false negatives when `root.dir` differs from the output directory (#2171), nor when only the converted path exists (#2459) (thanks, @naikymen @jameelalsalam @cderv, r-lib/pkgdown#2334).
