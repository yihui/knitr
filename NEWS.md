...
- Added a new function `label_chunks()` to assign labels to unnamed code chunks in a document, so that **knitr**'s messages, cache, and figure file names refer to meaningful labels instead of automatically generated ones like `unnamed-chunk-1`. The document syntax is detected automatically (e.g., R Markdown, R Sweave, or R HTML). The label for each unnamed chunk is generated by a function that can be customized via the `label` argument (thanks, @rkb965, #2228).
