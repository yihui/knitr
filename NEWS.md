...
- Added a new function `pkg_bib_nocite()` that returns the citation keys of the BibTeX entries generated by `write_bib()` (including any extra preferred citations a package provides beyond its auto-generated `R-` entry), formatted for the `nocite` field of the YAML metadata. This lists R packages in the References section without having to cite them explicitly in the text (thanks, @connorp, #1733).
