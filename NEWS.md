...
- HTML tables generated by `kable()` now give column headers the `scope="col"` attribute for accessibility. Row-name cells can additionally be marked up as row headers (`<th scope="row">`) by setting `options(knitr.table.html.row.header = TRUE)`; this is off by default because `<th>` cells may render differently from `<td>` (thanks, @tphilippi, #1747).
...
- `include_graphics()` gained a new argument `copy`. When `copy = TRUE`, the image files are copied into the figure directory of the current chunk (using the same naming scheme as the plots generated by the chunk) and the copies are included instead. This is useful when the images are in a temporary directory (e.g., generated by the **magick** package), which may not survive to the rendering step of output formats that do not embed images. The default can be set globally via `options(knitr.graphics.copy = TRUE)` (thanks, @jeroen, #1513).
...
- Added a `rust` engine to run Rust code chunks with extendr via the **rextendr** package, which requires a Rust toolchain (`cargo`). By default, the chunk is evaluated as Rust statements with `rextendr::eng_extendr()`, and the value of the last expression is printed. To compile Rust functions and export them to R instead, so that later R chunks can call them, set the chunk option `rust.source = TRUE` (this uses `rextendr::eng_extendrsrc()`) (#2524).
...
- An `Rcpp` chunk that reads its code from files via the `file` option (e.g., `file = c("foo.h", "foo.cpp")`) now compiles the files directly instead of their concatenated content, so a source file can `#include` its sibling header files. List the header(s) alongside the source file, and the last `.cpp`/`.cc` file is compiled (thanks, @astamm, #2367).
...
- The `<iframe>` generated by `include_url()` (and `include_app()`) now honors the chunk option `out.height`, consistent with `out.width`. When `out.height` is not set, the `height` argument passed to `include_url()` is used as before (thanks, @cderv, #1998).
