...
- Added a helper function `vignette_depends()` that reads the packages declared in `\VignetteDepends{}` of the vignette currently being built, and sets the chunk option `eval = FALSE` when any of them is not installed. This lets a vignette that illustrates optional (`Suggests`ed) packages skip evaluation gracefully when those packages are unavailable (e.g., on CRAN), by calling `knitr::vignette_depends()` in its first code chunk (thanks, @Enchufa2, #2164).
