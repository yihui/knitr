...
- **Assignment**: Use `=` instead of `<-` for assignment
...
Write all test conditions in `()`, use `%==%` to test for `identical()`, and test conditions can return vectors (if all elements are TRUE, the test passes).
   ```r
