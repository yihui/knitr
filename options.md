...
- `progress`: (`TRUE`) whether to display a progress bar when running **knitr**; note it also depends on the environment variable `R_KNITR_PROGRESS` (it this variable is set to `FALSE`, the `progress` option will be set to `FALSE` when **knitr** is loaded)
