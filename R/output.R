...
#'
#'   If the first code chunk of the document is not R but another language,
#'   `purl()` tangles the document to a script in that language instead, using
#'   the engine name as the file extension by default (e.g. \file{foo.py} for
#'   `python`; pass the `output` argument to override). Chunk headers become
#'   `#%%` code cells, and chunks of other languages are ignored.
#' @export
...
knit = function(
  ...
  if (in.file) {
    ...
    auto.output = (is.null(output) || is.na(output)) && !child_mode()
    if (auto.output)
  ...
  # when the first code chunk is not R, purl() tangles to a script in that
  # language, using the engine name as the file extension by default
  if (tangle && !child_mode()) {
    olang = .knitEnv$tangle.lang; on.exit({.knitEnv$tangle.lang = olang}, add = TRUE)
    .knitEnv$tangle.lang = detect_tangle_lang(text)
  }
  if (!is.null(lang <- .knitEnv$tangle.lang) && !child_mode()) {
    new.ext = tangle_lang_ext(lang)
    if (auto.output && is.character(output)) output = with_ext(output, new.ext)
    if (is.character(.knitEnv$tangle.file))
      .knitEnv$tangle.file = with_ext(.knitEnv$tangle.file, new.ext)
