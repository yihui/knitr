...
knit = function(input, output = NULL, tangle = FALSE, text = NULL, quiet = FALSE,
  ...
  # need text to determine pattern type and output format
  ...
  # need pattern/output format to determine output name
  apat = all_patterns; opat = knit_patterns$get()
  on.exit(knit_patterns$restore(opat), add = TRUE)
  if (length(opat) == 0 || all(vapply(opat[c('chunk.begin', 'inline.code')], is.null, logical(1)))) {
  ...
  # if no match, then out.format is NULL
  opts_knit$set(out.format = knit_patterns$get('out.format') %n% 'unknown')
  # check that a format is set (not NULL)
  if (opts_knit$get('out.format') == 'unknown') {
    warning('Cannot determine the output format.')
  }

  # get output name from pattern
  if (in.file) {
    if ((is.null(output) || is.na(output)) && !child_mode()) {
      output = basename(auto_out_name(input, tangle = tangle))
    }
  }

  # do not run purl() when the output is newer than input (the output might
  # have been generated by hook_purl)
  if (in.file && is.character(output) && !child_mode()) {
  ...
  # if file, then set tikzMetrics
  if (in.file && is.null(getOption('tikzMetricsDictionary'))) {
...
# knit's default output naming convenion
default_out_name = function(input) {
  base = sans_ext(input)
  ext = tolower(file_ext(input))
  name = if (grepl('_knit_', input)) {
    sub('_knit_', '', input)
  } else {
    if (ext == 'txt') {
      c(base, '-out.txt')
...
auto_out_name = function(input, pat = knit_patterns$get(),
                         tangle = opts_knit$get('tangle')) {
  base = sans_ext(input)
  ext = tolower(file_ext(input))
  name = if (tangle) {
    outname = paste0(base, '.R')
  } else {
    pat_ext = pat[['ext']]
    if (length(pat_ext)) {
      outext = pat_ext[[ext]]
      # if missing value, use default
      if (is.null(outext) || is.na(outext)) {
        outname = default_out_name(input)
      } else if (is.function(outext)) {
        outname = outext(input)
      } else {
        outname = paste0(base, as.character(outext))
      }
    } else {
      outname = default_out_name(input)
    }
