...
knit = function(input, output, pattern) {
    ...
    if (is.null(oopts$out.type)) {
        out.type =
            switch(ext, rnw = 'tex', tex = 'tex', html = 'html', md = 'jekyll',
                   stop('cannot automatically decide the output filetype'))
        ## set built-in hooks
        opts_knit$set(out.type = out.type)
    }
    switch(opts_knit$get('out.type'), tex = hooks_latex(), html = hooks_html(),
           jekyll = hooks_jekyll())
...
wrap.source = function(x, options) {
    ...
    src = x$src
    if (options$highlight && identical(opts_knit$get('out.type'), 'tex')) {
