...
- built-in cache: ideas like **cacheSweave** but **knit** directly uses `base::lazyLoad()` to fulfill lazy loading of objects, and another significant difference is that a cached chunk can still 'print' results (in **cacheSweave**, cached chunks no longer have any output, even you explicitly `print()` an object; **knit** actually caches the chunk output as well)
