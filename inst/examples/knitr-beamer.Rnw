...
% \SweaveOpts{prefix.string=figure/beamer-,align=center,fig.hold=TRUE,size=footnotesize}
...
\item Read \url{http://yihui.github.com/knitr/demo/lyx/}
\end{enumerate}
\item If you use Sweave or \textbf{knitr} with Beamer in \LyX{}, you probably
use the FragileFrame module too. Let's see if \textbf{knitr} works
with Beamer and FragileFrame in this small demo. 
\end{itemize}
...
<<boring-plots,fig=TRUE,width=4,height=4,out.width=.45\linewidth>>=
