...
```
and connect knitr to that kernel with the chunk option `kernel='existing'`. If you want to connect to other kernels than the last one started, execute 
```
%connect_info
```
...
Note that the variable `x` defined above is accessible from later chunks, even from inline chunks, with for instance: the value of `x` is `ipython x`, and 2 &times; &pi; = `ipython 2*pi`.
...
* document $\LaTeX$ + ipython in Rnw files
* can we fix the inline chunks?
