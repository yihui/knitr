...
<<cairo-png-dev,ref.label=test-plot,dev=Cairo_png,out.width=.5\linewidth,echo=FALSE>>=
@
...
by default. If you definitely want the background color to be always
white for base graphics, you can set up a hook for the \texttt{white}
option like this:
...
\begin{figure}
<<pdf-crop,ref.label=pdf-nocrop,echo=FALSE,fig.width=3.2,fig.height=3.2,out.width=.4\linewidth,crop=TRUE>>=
@
