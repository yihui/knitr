...
% \SweaveOpts{prefix.string=figure/manual-,prefix.cache=cache/manual-,align=center,external=TRUE,fig.show=hold,par=TRUE}
...
\begin{figure}
<<cars-demo,dev=tikz,width=4,height=2.8,out.width=.45\textwidth,message=FALSE,cache=TRUE>>=
fit=lm(dist~speed,data=cars) # linear regression
par(mar=c(4, 4, 1, .1), mgp=c(2,1,0),bg='white')
...
appropriate set of \href{http://yihui.github.com/knitr/patterns}{patterns}
to use if they are not provided in advance (e.g. \textsf{file.Rnw}
will use \texttt{knit\_patterns\$get('rnw')});
\item a set of out \href{http://yihui.github.com/knitr/hooks}{hooks} will
...
chunk depends on other chunks (see the \texttt{dependson} \href{http://yihui.github.com/knitr/options}{option})
...
You may check out a number of specific manuals dedicated to specific
features such as graphics in the website: \url{http://yihui.github.com/knitr/demos}.
...
borrowed from the \textbf{highlight} package by \href{http://www.andre-simon.de/}{Andre Simon}%
\footnote{not the R package mentioned before; for a preview of these themes,
see \url{http://www.andre-simon.de/dokuwiki/doku.php?id=theme_examples}%
}; it is also possible to use themes from \url{http://www.eclipsecolorthemes.org/}
by providing a theme id to \textbf{knitr}%
\footnote{many thanks to \href{https://github.com/ramnathv}{Ramnath Vaidyanathan}
...
<<low-level-plots,fig.keep=all,dev=tikz,width=2.5,height=2.5,out.width=.3\textwidth,cache=TRUE>>=
par(mar=c(3,3,.1,.1))
...
<<fig-hold,fig=TRUE,fig.show=asis,dev=pdf,width=6,height=4,out.width=.35\linewidth>>=
...
<<math-expr-tikz,dev=tikz,width=5,height=3,out.width=.55\linewidth,cache=TRUE,message=FALSE>>=
library(ggplot2)
qplot(1:10, 1:10) + 
...
check it out in the \href{http://yihui.github.com/knitr/demo/graphics/}{website}.
...
a directory (e.g. \texttt{prefix.cache=cache/abc-}), cache files will
be stored under that directory (automatically created if it does not
exist). The cache is invalidated and purged on any changes to the
code chunk, including both the R code and chunk options; this means
previous cache files of this chunk are removed (filenames are identified
by the chunk label). Unlike \textbf{pgfSweave}, cache files will never
accumulate since old cache files will always be removed in \textbf{knitr}.
Unlike \textbf{weaver} or \textbf{cacheSweave}, \textbf{knitr} will
try to preserve side-effects, meaning that any output of a code chunk
will be loaded into the output document for a cached chunk, although
it is not really evaluated. The reason is \textbf{knitr} also cache
the output of a chunk as a character string. Note this means graphics
output is also cached since it is part of the output. It has been
a pain for me for a long time to have to lose output to gain cache.
...
otherwise the current chunk has to be re-evaluated and its cache will
be rebuilt. The way I implement \texttt{dependson} is that a file
named in the form \texttt{chunk-label\_MD5\_changed} is used to indicate
if a chunk was changed; this file is created each time cached objects
are saved, and removed the next time when cache is loaded, so if this
file does not exist, it means nothing was changed in this chunk.
...
Different documents can refer to the same R script, so the R code
can be reusable across different input documents.


\subsection{Conditional Evaluation\label{sub:conditional}}

Instead of setting chunk options \texttt{eval} and \texttt{echo} to
be \texttt{TRUE} or \texttt{FALSE}, their values can be controlled
by global variables in the current R session. This enables \textbf{knitr}
to conditionally evaluate code chunks according to variables. For
example, here we assign \texttt{TRUE} to a variable \texttt{dothis}:
...
in HTML, it is only a matter of putting output in a \texttt{div} layer.
These are simply character string operations. The demo of using the
\textbf{listings} package in the \href{http://yihui.github.com/knitr/demo/listings/}{website}
...
is not enough, please let me know, or set it in chunk options by yourself);
note only one device can be used for a chunk (in old ages, we may
have to use both \texttt{eps} and \texttt{pdf}, but nowadays there
is usually no need to generate multiple image formats for the same
plot)
\item [{fig}] was dropped; now use \texttt{fig.keep=high}
\end{description}
For logical options, only \texttt{TRUE}/\texttt{FALSE}/\texttt{T}/\texttt{F}
are supported (the first two are recommended), and \texttt{true}/\texttt{false}
will not work, e.g., \texttt{eval=FALSE} is OK, and \texttt{eval=false}
is not (not exactly true; see Section \ref{sub:conditional}). Chunk
reference using \texttt{<\textcompwordmark{}<chunk-label>\textcompwordmark{}>}
...
For inline R code as in \texttt{\textbackslash{}Sexpr\{\}}, \textbf{knitr}
will automatically format the results -- numbers will be written in
real scientific notations (e.g. $3.14\times10^{-5}$ instead of the
less readable \texttt{3.14e-05}). We can also call \texttt{knit(...,
tangle = TRUE)} to tangle R code, which is similar to \emph{Stangle()}.
...
figures will not be generated at all, and it is safe to use \texttt{fig.keep}
(close to \texttt{fig=TRUE} in Sweave) even if there are not plots
in a chunk
...
many digits)
\item [{A.13}] \textbf{knitr} comes with a set of hooks to deal with HTML
output (see \href{http://yihui.github.com/knitr/demos}{demos in the website})
\end{description}
Currently the multilingual support is limited, especially under Windows
(I do not have much experience with encodings). For systems which
use the UTF8 locale, \textbf{knitr} works fine for multi-byte characters.
Note, however, \textbf{knitr} does not check the encoding of the input
document at all, so the tricks in Sweave like using \texttt{\textbackslash{}usepackage{[}foo{]}\{inputenc\}}
will not work.
