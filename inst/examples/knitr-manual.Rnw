...
% \SweaveOpts{fig.path=figure/manual-,cache.path=cache/manual-,fig.align=center,external=TRUE,fig.show=hold,par=TRUE}
...
enhancements and new features will be introduced later.
...
<<fig-hold,fig=TRUE,fig.show=asis,dev=pdf,fig.width=6,fig.height=4,out.width=.35\linewidth>>=
...
to the code chunk, including both the R code and chunk options%
\footnote{One exception is the \texttt{include} option, which is not cached
...
option affects the width of text output)%
}; this means previous cache files of this chunk are removed (filenames
...
Therefore \textbf{knitr} introduced the feature of code externalization
to a separate R script. Currently the setting is like this: the R
...
R script to input external R code. For example, suppose this is a
code chunk labelled as \texttt{Q1} in an R script named \textsf{homework1-xie.R}
...
<<fancy-rgl,rgl=TRUE,dev=png,fig.width=5,fig.height=5,out.width=2in,message=FALSE,cache=TRUE>>=
library(rgl)
...
hooks)
\item [{prefix}] was dropped (think \texttt{prefix=TRUE})
\item [{prefix.string}] was renamed to \texttt{fig.path} and it is always
used for figure filenames
...
plot)
\item [{fig}] was dropped; now use \texttt{fig.keep}: \texttt{fig.keep=high}
is equivalent to \texttt{fig=TRUE} and \texttt{fig.keep=none} is the
same as \texttt{fig=FALSE}
\item [{width,~height}] were dropped; use \texttt{fig.width} and \texttt{fig.height}
instead
\end{description}
...
was dropped; if you want to reuse a chunk, either consider writing
a function instead of explicitly repeating code, or make use of code
externalization as introduced before, or use the new option \texttt{ref.label}
(see the demo ``\href{http://yihui.github.com/knitr/demo/reference/}{Chunk Reference}''
in the website).
