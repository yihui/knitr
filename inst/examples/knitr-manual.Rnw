...
is still available, and there are other approaches for reusing chunks,
e.g., use the new option \texttt{ref.label} or the function \emph{run\_chunk()}
...
will automatically format the results -- too big or too small numbers
will be written in scientific notations (e.g. $3.14\times10^{-5}$
instead of the less readable \texttt{3.14e-05}). We can also call
\texttt{purl()} to extract R code, which is similar to \emph{Stangle()}.
