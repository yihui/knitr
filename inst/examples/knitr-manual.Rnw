...
The \textbf{highlight} package \citep{R-highlight} will be used to
highlight R code if it is installed (otherwise an internal naive syntax
highlighter is used), and \textbf{formatR} \citep{R-formatR} is used
