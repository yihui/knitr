...
% \SweaveOpts{prefix.string=figure/manual-,align=center,external=TRUE,fig.hold=TRUE}

<<setup,echo=FALSE,results=hide,message=FALSE>>=
options(replace.assign=TRUE,width=90)
hooks$set(fig=function(before, options, envir){if (before) par(mar=c(4,4,.1,.1),cex.lab=.95,cex.axis=.9,mgp=c(2,.7,0),tcl=-.3)})
@
...
\href{http://www.cs.tufts.edu/~nr/noweb/}{Noweb}; Sweave \citet{leisch2002}
...
(e.g. \textsf{file.Rnw} will use \texttt{kpat\$get('rnw')});
...
The paths of cache files are determined by the chunk option \texttt{prefix.string};
by default all cache files are created under the current working directory,
and if the prefix string contains a directory (e.g. \texttt{prefix.string=cache/abc-}),
cache files will be stored under that directory. The cache is invalidated
and purged on any changes to the code chunk, including both the R
code and chunk options. Unlike \textbf{weaver} or \textbf{cacheSweave},
...
hooks$set(rgl = function(before, options, envir) {
    ...
    if (rgl.cur() == 0) return()  # no active device
        name = paste(options$prefix.string, options$label, sep = '')
    rgl.snapshot(paste(name, '.png', sep = ''), fmt = 'png')
    return(paste('\\\\centering{}\\\\includegraphics[', sprintf('width=%s',options$out.width), ']{', name, '}\n', sep = ''))
    }
})
@
...
<<fancy-rgl, rgl=TRUE, out.width=2in,message=FALSE,cache=TRUE>>=
library(rgl)
...
externalization;
\item For the latter, \texttt{cache} is implemented with functions in base
