...
- add `*.Rmarkdown` files under the `vignettes` directory (in theory, we can use `*.Rmd` as well, but currently there is a bug in R 3.0.1, so we have to use a different file extension)
- add `Suggests: knitr` and `VignetteBuilder: knitr` to the `DESCRIPTION` file
- specify the vignette engine `\VignetteEngine{knitr::docco_linear}` in the `Rmarkdown` files (inside HTML comments)
