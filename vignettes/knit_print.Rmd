...
After we evaluate each R expression in a code chunk, there is an object returned. For example, `1 + 1` returns `2`. This object is passed to the chunk option `render`, which is a function with one argument. The default value for the `render` option is `knit_print`, an S3 function in **knitr**:
...
As we can see, `knit_print()` only has the `default` method, which is basically `print()` or `show()`, depending on whether the object is an S4 object. This means it does nothing special when printing R objects:
...
```{r}
dummy_print = function(x, options) {
...
```{r tidy=FALSE}
library(knitr)
knit_print.foo = function(x, options) {
...
```{r}
knit_print.bar = function(x, options) {
