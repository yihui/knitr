...
\title{Not an Introduction to knitr}
...
\footnote{e.g. the main manual: \url{https://bitbucket.org/stat/knitr/downloads/knitr-manual.pdf}
and the graphics manual: \url{https://bitbucket.org/stat/knitr/downloads/knitr-graphics.pdf}%
...
with \textbf{knitr} as well using R 3.0.x, which supports non-Sweave
vignettes:
...
\bibliography{../inst/examples/knitr-packages}
