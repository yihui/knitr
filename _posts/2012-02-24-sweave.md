...
Note **knitr** does not check the encoding of the input document at all, so the tricks in Sweave like using `\usepackage[foo]{inputenc}` will not work. You have to specify the encoding explicitly if you are not using the native encoding in your system, e.g. `knit('foo.Rnw', encoding = 'GBK')`.
