...
3. `source()` a script (**knitr** is unable to know which objects are created by `source()`)
...
It is generally not recommended to use `source()` in a cached chunk, but it is safe to call it through a custom hook and a chunk option (say, `script`) like this:
