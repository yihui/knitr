...
If the output file type is LaTeX, default hooks will put most output in the `verbatim` environment, and `inline` output will be put in `\texttt{...}`; `plot` and `chunk` hooks are more complicated:
